...
PACKAGE_ROOT = Path(__file__).resolve().parent.parent / "agent_broker_v2"
...
class V2ConfigTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        sys.modules.pop("agent_broker_v2.config_local", None)
        sys.modules.pop("agent_broker_v2.config", None)
    ...
    def test_config_uses_defaults_when_no_local_override_exists(self) -> None:
        importlib.invalidate_caches()
        config = importlib.import_module("agent_broker_v2.config")

        self.assertEqual(
            config.BROKER_CFG_V2,
            {
    ...
    def test_config_local_overrides_base_config(self) -> None:
        CONFIG_LOCAL_PATH.write_text(
            'BROKER_CFG_V2 = {"allowlist": ["python3"], "extra": {"enabled": True}}\n',
            encoding="utf-8",
        )
