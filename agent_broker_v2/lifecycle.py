...
def create_session(base_dir: str | Path | None = None) -> SessionPaths:
    ...
    session_dir = Path(
        tempfile.mkdtemp(prefix="agent-broker-v2-", dir=str(root))
    ).resolve()
...
def ensure_session_shims(paths: SessionPaths, tool_names: list[str]) -> None:
    paths.shim_bin_dir.mkdir(mode=0o700, exist_ok=True)
    shim_target = Path(__file__).resolve().parent.parent / "cli" / "shim_cli_v2.py"
...
def build_allowlist_from_config() -> dict[str, ToolSpec]:
    candidates = {
        ...
        for tool_name in BROKER_CFG_V2["allowlist"]
    }
    return {
...
def print_manual_instructions(
    ...
    shim_cli_path = Path(repo_root).resolve() / "cli" / "shim_cli_v2.py"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the v2 broker for one manual client session."
    )
    parser.add_argument(
        "--session-dir",
    ...
    broker = Broker(paths=paths, allowed_tools=allowed_tools)
    return run_manual_session(broker=broker, repo_root=repo_root)
